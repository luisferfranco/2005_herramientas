...
tip = d3
        ...
        .attr('class', 'd3-tip')
        .html(d => {
          txt = `<strong><span style="text-transform:capitalize">${d.country}</strong><br>`
          txt += `GDP Per Capita: <strong><span style="color:red;">${d3.format(',.0f')(d.income)}</span></strong><br>`
          txt += `Population: <strong><span style="color:red;">${d3.format(',.0f')(d.population)}</span></strong>`
          return txt
        })
g.call(tip)
